export default function LiveClassVideoStage({ teacher, participants }) {
  return (
    <section className="flex flex-1 flex-col relative bg-[#f6f7f8] p-4 sm:p-6 gap-4 sm:gap-6 overflow-hidden w-full">
      {/* Main Video Feed (Teacher/Screen Share) */}
      <div className="flex-1 relative rounded-xl overflow-hidden shadow-sm ring-1 ring-slate-200 bg-black group isolate">
        {/* Main Image */}
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 sm:gap-4 bg-white/90 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/20 px-4 sm:px-6 py-2.5 rounded-full z-20 transition-all hover:scale-105">
          <div className="flex items-center gap-2 pr-4 border-r border-slate-200">
            <button className="size-10 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors tooltip" title="Mute Microphone">
              <span className="material-symbols-outlined text-[20px]">mic</span>
            </button>
            <button className="size-10 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors tooltip" title="Stop Camera">
          ...
          <div className="pl-4 border-l border-slate-200 flex flex-col items-start justify-center gap-0.5">
            ...
            <label className="relative inline-flex items-center cursor-pointer group/toggle">
              ...
              <span className="ml-2 text-xs font-bold text-green-600 peer-checked:opacity-100 opacity-50">ON</span>
            </label>
          </div>
        </div>
      </div>
      ...
      <div className="h-28 sm:h-36 shrink-0 flex gap-3 sm:gap-4 overflow-x-auto pb-2 scrollbar-hide px-1">
        {participants.map((participant) => (
          <div key={participant.id} className={`relative aspect-video h-full rounded-lg overflow-hidden bg-slate-800 ${participant.isActiveSpeaker ? 'ring-2 ring-primary ring-offset-2 ring-offset-[#f6f7f8] shadow-md' : 'shadow-sm'} transition-transform hover:-translate-y-1`}>
        ...
        <div className="relative aspect-video h-full rounded-lg overflow-hidden bg-slate-200 flex flex-col items-center justify-center text-slate-500 border border-slate-300 hover:bg-slate-300 cursor-pointer transition-colors">
