...
export default function ClassStudentViewByTutor() {
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark min-h-screen text-[#0d141b] dark:text-slate-50">
            <div className="relative flex h-auto min-h-screen w-full flex-col group/design-root overflow-x-hidden">
                <div className="layout-container flex h-full grow flex-col">
                    {/* Top Navigation Bar */}
                    <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-[#e7edf3] dark:border-slate-800 bg-white dark:bg-[#1a2632] px-10 py-3">
                        <div className="flex items-center gap-8">
                            ...
                            <div className="flex items-center gap-9">
                                <a className="text-[#0d141b] dark:text-slate-300 text-sm font-medium leading-normal" href="#">Dashboard</a>
                                <a className="text-primary text-sm font-semibold leading-normal" href="#">Classes</a>
                                <a className="text-[#0d141b] dark:text-slate-300 text-sm font-medium leading-normal" href="#">Schedule</a>
                                <a className="text-[#0d141b] dark:text-slate-300 text-sm font-medium leading-normal" href="#">Resources</a>
                            </div>
                        </div>
                        <div className="flex flex-1 justify-end gap-8">
                            <label className="flex flex-col min-w-40 !h-10 max-w-64">
                                <div className="flex w-full flex-1 items-stretch rounded-lg h-full">
                                    ...
                                    <input
                                        className="form-input flex w-full min-w-0 flex-1 border-none bg-[#e7edf3] dark:bg-slate-800 text-[#0d141b] dark:text-white focus:outline-0 focus:ring-0 h-full placeholder:text-[#4c739a] px-4 rounded-r-lg text-sm font-normal"
                                        placeholder="Search"
                                    />
                                </div>
                            </label>
                            <div className="flex gap-2">
                                <button className="flex items-center justify-center rounded-lg size-10 bg-[#e7edf3] dark:bg-slate-800 text-[#0d141b] dark:text-white">
                    ...
                    <main className="flex flex-1 justify-center py-6 px-10">
                        <div className="layout-content-container flex flex-col max-w-[1200px] flex-1">
                            ...
                            <div className="flex flex-wrap gap-2 items-center mb-2">
                                {breadcrumbs.map((crumb, index) => (
                                    <React.Fragment key={index}>
                                        {crumb.active ? (
                                            <span className="text-[#0d141b] dark:text-white text-sm font-semibold">{crumb.label}</span>
                                        ) : (
                                            <>
                            ...
                            <div className="flex justify-between items-end mb-6">
                                <h1 className="text-[#0d141b] dark:text-white tracking-tight text-3xl font-bold">{className}</h1>
                                <div className="flex gap-2">
                            ...
                            {/* Tabs */}
                            <div className="border-b border-[#cfdbe7] dark:border-slate-800 mb-6">
                            ...
                            <div className="flex gap-8">
                                ...
                                <div className="flex-1 flex flex-col gap-6 min-w-0">
                                    ...
                                    <div className="bg-white dark:bg-[#1a2632] border border-[#e7edf3] dark:border-slate-800 rounded-xl p-5 shadow-sm">
                                        ...
                                        <div className="flex items-end gap-2 h-16 w-full">
                                            {attendance.weekData.map((day, index) => (
                                                <div
                                                    key={index}
                                                    className={`flex-1 ${day.percentage > 0 ? 'bg-primary/20' : 'bg-slate-100 dark:bg-slate-700'} rounded-t h-[80%] relative group`}
                                                    title={day.label}
                                                >
                                                    {day.percentage > 0 && (
                                                        <div
                                    ...
                                    <div className="flex flex-wrap items-center justify-between gap-4">
                                        <div className="flex items-center gap-3 flex-1 min-w-[300px]">
                                            <div className="relative flex-1">
                                                ...
                                                <input
                                                    className="w-full bg-white dark:bg-[#1a2632] border-[#e7edf3] dark:border-slate-800 rounded-lg pl-10 pr-4 py-2 text-sm focus:ring-primary focus:border-primary"
                                            ...
                                            <div className="relative">
                                                <select
                                                    className="appearance-none bg-white dark:bg-[#1a2632] border-[#e7edf3] dark:border-slate-800 rounded-lg px-4 py-2 pr-10 text-sm font-medium text-[#4c739a] focus:ring-primary focus:border-primary cursor-pointer"
                                                    value={statusFilter}
                                    ...
                                    {/* Student Table */}
                                    <div className="bg-white dark:bg-[#1a2632] border border-[#e7edf3] dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-slate-50 dark:bg-slate-800/50 text-[#4c739a] text-[11px] font-bold uppercase tracking-wider">
                                                    ...
                                                    <tr key={student.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                                        <td className="px-6 py-4">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                />
                                                                <div>
                                                                    <p className="text-sm font-semibold text-[#0d141b] dark:text-white leading-tight">{student.name}</p>
                                        ...
                                        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                            ...
                                            <div className="flex gap-2">
                                                <button className="px-3 py-1 text-xs font-bold text-[#4c739a] border border-[#e7edf3] dark:border-slate-700 rounded bg-white dark:bg-slate-800">Prev</button>
                                                <button className="px-3 py-1 text-xs font-bold text-[#4c739a] border border-[#e7edf3] dark:border-slate-700 rounded bg-white dark:bg-slate-800">Next</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <aside className="w-80 flex flex-col gap-6">
                                        ...
                                        <div className="flex flex-col gap-4">
                                            {alerts.map((alert) => (
                                                ...
                                                >
                                                    ...
                                                    />
                                                    <div className="flex flex-col gap-1">
                                                        <p className={`text-xs font-bold ${alert.severity === 'high' ? 'text-red-700 dark:text-red-400' : 'text-amber-700 dark:text-amber-400'
                                                            }`}>
                                                            {alert.studentName}
                                                        </p>
                                                        <p className={`text-[11px] leading-tight ${alert.severity === 'high' ? 'text-red-600 dark:text-red-300' : 'text-amber-600 dark:text-amber-300'
                                                            }`}>
                                                            {alert.message}
                                                        </p>
                                                        <button className={`mt-1 text-[10px] font-bold underline uppercase ${alert.severity === 'high' ? 'text-red-700 dark:text-red-400' : 'text-amber-700 dark:text-amber-400'
                                                            }`}>
                                                            {alert.action}
                                                        </button>
                                                    </div>
                                    ...
                                    {/* Quick Stats */}
                                    <div className="bg-white dark:bg-[#1a2632] border border-[#e7edf3] dark:border-slate-800 rounded-xl p-5 shadow-sm">
                                        <h3 className="text-sm font-bold text-[#0d141b] dark:text-white uppercase tracking-wider mb-4">
                                            Quick Stats
                                        </h3>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="flex flex-col p-3 bg-background-light dark:bg-slate-800/50 rounded-lg">
